...
struct DuplicationState {
    ...
    /// CPU-readback pipeline, built lazily only when
    /// `cpu_nv12_readback_enabled()`. `input` is a persistent BGRA texture the
    /// per-frame desktop copy lands in; `converter` reads its NV12 output back
    /// to CPU. Rebuilt only on a dimension change.
    nv12_readback: Option<DxgiNv12Readback>,
    cap_w: u32,
    cap_h: u32,
    out_w: u32,
    out_h: u32,
...
#[cfg(target_os = "windows")]
struct DxgiNv12Readback {
    /// Held to keep the converter's input view alive; not read directly (the
    /// per-frame desktop copy targets `input_resource`, a view of this texture).
    _input_texture: ID3D11Texture2D,
    input_resource: ID3D11Resource,
    converter: crate::nv12_gpu::Nv12GpuConverter,
    width: u32,
    height: u32,
}
...
fn emit_acquired_frame(
    ...
    if crate::cpu_nv12_readback_enabled() {
        return emit_dxgi_cpu_nv12_frame(
            inner,
            device,
            context,
            state,
...
/// CPU-readback emit path for the DXGI duplication fallback. Copies the cropped
/// desktop region into a persistent BGRA intermediate, converts it to GPU NV12,
/// reads that back to CPU, and emits `ScreenFrame::Nv12`. Used only when
/// `cpu_nv12_readback_enabled()` (the DXGI path is 1:1, so no scaling).
#[cfg(target_os = "windows")]
#[allow(clippy::too_many_arguments)]
fn emit_dxgi_cpu_nv12_frame(
    inner: &Arc<CaptureInner>,
    device: &ID3D11Device,
    context: &ID3D11DeviceContext,
    state: &mut DuplicationState,
    ...
    let cap_w = state.cap_w;
    let cap_h = state.cap_h;
    if let Err(e) = ensure_dxgi_nv12_readback(device, context, state) {
        return FrameResult::Error(e);
    }
    let Some(readback) = state.nv12_readback.as_mut() else {
        return FrameResult::Error("DXGI NV12 readback pipeline unavailable".into());
    };
    // Land the cropped desktop region at (0,0) in the intermediate BGRA texture.
    unsafe {
        context.CopySubresourceRegion(
            &readback.input_resource,
            0,
            0,
            0,
            0,
    ...
    let cpu = match readback.converter.convert_to_cpu_nv12() {
        Ok(cpu) => cpu,
        Err(e) => return FrameResult::Error(e),
    };
    assert_eq!(cpu.width, cap_w, "DXGI NV12 readback width matches capture");
    assert_eq!(
        cpu.height, cap_h,
        "DXGI NV12 readback height matches capture"
    );
    let _ = crate::emit_nv12_cpu_frame(
        inner,
        frame_sink,
        cpu,
        capture_timestamp_us(capture_start),
...
/// Build (or rebuild on a dimension change) the DXGI NV12 CPU-readback pipeline:
/// a persistent BGRA input texture the desktop copy lands in, plus an
/// `Nv12GpuConverter` bound to it. Reused across frames on the hot path.
#[cfg(target_os = "windows")]
fn ensure_dxgi_nv12_readback(
    device: &ID3D11Device,
    context: &ID3D11DeviceContext,
    state: &mut DuplicationState,
) -> Result<(), String> {
    ...
    assert!(width > 0, "DXGI NV12 input width positive");
    assert!(height > 0, "DXGI NV12 input height positive");
    ...
    unsafe { device.CreateTexture2D(&input_desc, None, Some(&mut input_texture)) }
        .map_err(|e| format!("CreateTexture2D DXGI NV12 input: {e}"))?;
    let input_texture =
        input_texture.ok_or_else(|| "CreateTexture2D DXGI NV12 input returned null".to_string())?;
    let input_resource: ID3D11Resource = input_texture
        .cast()
        .map_err(|e| format!("ID3D11Resource DXGI NV12 input cast: {e}"))?;
    let converter = crate::nv12_gpu::Nv12GpuConverter::new(
        device,
        context,
        &input_texture,
        width,
    ...
    .ok_or_else(|| "DXGI NV12 converter unavailable".to_string())?;
    state.nv12_readback = Some(DxgiNv12Readback {
