...
fn emit_shared_texture_to_bus(
    ...
    if let Some(nv12) = reader.nv12.as_mut() {
        if crate::cpu_nv12_readback_enabled() {
            match nv12.convert_to_cpu_nv12() {
                Ok(cpu) => {
                    session.set_native_texture_info(None);
                    return Ok(crate::emit_nv12_cpu_frame(
                        inner,
                        frame_sink,
                        cpu,
                        timestamp_us,
                ...
                Err(error) => {
                    verbose_log(&format!(
                        "disabling GPU NV12 CPU-readback for this shared texture after failure: \
                         {error}"
                    ));
                    reader.nv12 = None;
                    session.set_native_texture_info(None);
            ...
            match nv12.convert_shared_texture() {
                ...
                Err(error) => {
                    verbose_log(&format!(
                        "disabling GPU NV12 conversion for this shared texture after failure: \
                         {error}"
                    ));
                    reader.nv12 = None;
                    session.set_native_texture_info(None);
