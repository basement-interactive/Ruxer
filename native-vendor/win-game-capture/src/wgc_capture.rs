...
pub fn parse_monitor_source_id(source_id: &str, source_kind: &str) -> Option<HMONITOR> {
    ...
    // The middle token is the raw HMONITOR value the picker embedded (see
    // src-tauri/src/screen_sources.rs). Parse it and confirm it is still one of
    // the currently-live monitors before trusting it — this avoids the
    // index-instability bug (two independent EnumDisplayMonitors passes can
    // disagree on order) while guarding against a stale handle after a display
    // reconfig. Fall back to treating the token as an enumeration INDEX for
    // backward-compat with any old `screen:N:0` id still in flight.
    let live = enumerate_monitors();
    if let Ok(raw) = token.parse::<isize>() {
        let as_handle = HMONITOR(raw as *mut core::ffi::c_void);
        if live.iter().any(|m| m.0 == as_handle.0) {
            return Some(as_handle);
        }
        // Backward-compat: old index-based id.
        if let Ok(index) = token.parse::<usize>() {
            if index < WGC_MONITOR_ENUM_LIMIT {
                return live.get(index).copied();
            }
        }
    }
    None
...
fn setup_wgc_state(ctx: &WgcLoopContext) -> Result<WgcState, String> {
    ...
    // FLUXER: prefer BGRA (SDR Rec.709) over FP16. The FP16 path is hardcoded to
    // `hdr: true` in create_wgc_output_pipeline, which on an ordinary SDR desktop
    // runs a linear-scRGB/HDR colour conversion and tags the NV12 output as a
    // colour space stock libwebrtc's encoder does not expect — producing the
    // GREEN cast watchers saw. BGRA goes through the plain SDR Rec.709→BT.601
    // converter (same path the correct-looking self-preview uses). FP16 stays as
    // a fallback only if BGRA pool creation fails (genuine HDR-only surfaces).
...
fn create_wgc_output_pipeline(
    ...
    // When a CPU consumer is active there is no zero-copy shared-texture path,
    // so always route BGRA content through the NV12 converter too (even at 1:1
    // scale): every frame then has a GPU NV12 texture to read back to CPU.
    if out_w != content_width || out_h != content_height || crate::cpu_nv12_readback_enabled() {
        return create_wgc_nv12_pipeline(
            ctx,
...
fn emit_wgc_nv12_frame(
    ...
    if crate::cpu_nv12_readback_enabled() {
        let cpu = match pipeline.converter.convert_to_cpu_nv12() {
            Ok(cpu) => cpu,
            Err(error) => return WgcFrameResult::Error(error),
        };
        let _ = crate::emit_nv12_cpu_frame(inner, frame_sink, cpu, timestamp_us);
        return WgcFrameResult::Ok;
    }
