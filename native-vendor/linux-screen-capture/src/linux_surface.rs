...
//! Plain-Rust Linux screen-capture surface (PipeWire + xdg-desktop-portal).
//!
//! This module was originally a napi (Node/Electron) native addon. The napi
//! surface has been stripped so the crate can be consumed as a plain Rust
//! `rlib` from the Tauri backend. The capture core (`portal`, `pipewire_stream`,
//! `game_capture`) is unchanged; only the FFI shell was replaced with a
//! pure-Rust API.
//!
...
//!
//! Portal picker note: on Wayland the *compositor* owns the source picker.
//! [`list_sources`] opens a portal ScreenCast session and drives the
//! CreateSession → SelectSources → Start handshake; the compositor shows its
//! own dialog and the user's choice comes back as the returned stream list.
//! The app cannot pre-pick a source id. The live session is parked and adopted
//! by the immediately-following [`ScreenCapture::start`] call.
...
use fluxer_screen_frame_bus::{
    ...
    NativeScreenFrameSinkHandleRef, Nv12Frame as BusNv12Frame, ScreenFrame as BusScreenFrame,
};

use crate::game_capture::{GameCaptureDiagnostics, GameCaptureVideoStream};
use crate::pipewire_stream::{
    FrameCallback, LINUX_FRAME_BYTES_MAX, LINUX_FRAME_DIM_MAX, LifecycleCallback as PwLifecycleCb,
    LinuxFrameBufferPool, PipeWireVideoStream, PoolExhaustionCallback, VideoFrame, daemon_reachable,
};
...
/// Open a portal ScreenCast session and drive the picker.
///
/// On Wayland the compositor renders the source-selection dialog; the returned
/// list reflects whatever the user picked. The live session is parked so that
/// the immediately-following [`ScreenCapture::start`] can adopt it (the portal
/// requires OpenPipeWireRemote to happen on the same session that was picked).
///
/// This performs blocking D-Bus I/O and shows a modal compositor dialog — the
/// caller should run it off the UI thread.
pub fn list_sources() -> Result<Vec<LinuxScreenCaptureSource>, String> {
    let (session, streams) = portal::open_session_and_pick().map_err(|err| {
...
impl ScreenCapture {
    ...
    {
        let cb: LifecycleCallback = Arc::new(callback);
        let mut state = lock_state(&self.inner);
        state.lifecycle_callback = Some(cb);
    }
    ...
    pub fn set_frame_sink_handle(&self, sink: Arc<NativeScreenFrameSinkHandleRef>) {
        let mut guard = lock_native_sink(&self.inner);
        *guard = Some(sink);
    }
    ...
    pub fn start(
        ...
        if source_id.is_empty() {
            return Err("ScreenCapture.start sourceId must be non-empty".to_string());
        }
        if source_kind != "screen" && source_kind != "window" && source_kind != "game" {
            return Err(
                "ScreenCapture.start sourceKind must be 'screen', 'window', or 'game'".to_string(),
            );
        }
        let start_options = normalize_start_options(capture_options)?;

        {
            let mut guard = lock_capture_id(&self.inner);
            *guard = capture_id
                .map(|raw| raw.trim().to_string())
                .filter(|trimmed| !trimmed.is_empty());
        }

        let lifecycle_callback = {
            let state = lock_state(&self.inner);
            state.lifecycle_callback.clone()
        };
        ...
        let capture_id_value = lock_capture_id(&self.inner).clone();
        let native_frame_sink = lock_native_sink(&self.inner).clone();
        ...
        let lifecycle_cb: PwLifecycleCb = Arc::new(move |kind: &str, message: &str| {
            if kind == "closed" || kind == "closed-clean" {
                inner_running.store(false, Ordering::Release);
            }
            let Some(cb) = lifecycle_callback.as_ref() else {
                return;
            };
            cb(kind.to_string(), message.to_string());
        });
        ...
        if source_kind == "game" {
            let game_stream = GameCaptureVideoStream::open(effective_fps, frame_cb, lifecycle_cb)
                .map_err(|e| format!("obs-vkcapture game stream open failed: {e}"))?;
            {
                let mut state = lock_state(&self.inner);
                state.session = None;
                state.stream = None;
        ...
        let node_id = parse_source_id(&source_id)
            .ok_or_else(|| "ScreenCapture.start sourceId must be a positive u32 string".to_string())?;
        let parked = adopt_session().ok_or_else(|| {
            "no live portal session — call listSources() immediately before start()".to_string()
        })?;
        ...
        let stream =
            PipeWireVideoStream::open(fd, node_id, frame_cb, lifecycle_cb, pool, exhaust_cb)
                .map_err(|e| format!("PipeWire stream open failed: {e}"))?;
    ...
    pub fn stop(&self) -> Result<(), String> {
        ...
        lock_capture_id(&self.inner).take();
        lock_native_sink(&self.inner).take();
        ...
        if was_running {
            emit_lifecycle(&self.inner, "closed-clean", "capture stopped");
        }
        Ok(())
    }
    ...
    pub fn get_diagnostics(&self) -> Option<ScreenCaptureDiagnostics> {
        ...
        if let Some(game_stream) = state.game_stream.as_ref() {
            return Some(game_stream.diagnostics().into());
        }
...
impl Drop for ScreenCapture {
    fn drop(&mut self) {
        ...
        lock_native_sink(&self.inner).take();
        let (stream, game_stream, session) = {
            let mut s = lock_state(&self.inner);
            (s.stream.take(), s.game_stream.take(), s.session.take())
        };
        drop(stream);
        drop(game_stream);
        if let Some(s) = session {
...
fn lock_state(inner: &CaptureInner) -> std::sync::MutexGuard<'_, CaptureState> {
    inner
        .state
        .lock()
...
fn lock_capture_id(inner: &CaptureInner) -> std::sync::MutexGuard<'_, Option<String>> {
    inner
        .capture_id
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
...
fn lock_native_sink(
    inner: &CaptureInner,
) -> std::sync::MutexGuard<'_, Option<Arc<NativeScreenFrameSinkHandleRef>>> {
    inner
        .native_frame_sink
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
...
) -> Result<NormalizedStartOptions, String> {
...
fn normalize_capture_rect(
...
) -> Result<Option<NormalizedCaptureRect>, String> {
    let Some(rect) = rect else {
        return Ok(None);
    };
    if !rect.x.is_finite() || !rect.y.is_finite() {
        return Err("ScreenCapture.start captureRect origin must be finite".to_string());
    }
    if !rect.width.is_finite() || !rect.height.is_finite() {
        return Err("ScreenCapture.start captureRect size must be finite".to_string());
    }
    ...
    if rect.width > u32::MAX as f64 || rect.height > u32::MAX as f64 {
        return Err("ScreenCapture.start captureRect width/height exceed u32".to_string());
    }
...
fn normalize_color_range(value: Option<&str>) -> Result<Option<LinuxColorRange>, String> {
    match value {
        None => Ok(None),
        Some("full") => Ok(Some(LinuxColorRange::Full)),
        Some("limited") => Ok(Some(LinuxColorRange::Limited)),
        Some(_) => Err("ScreenCapture.start colorRange must be 'limited' or 'full'".to_string()),
    }
}

fn normalize_color_space(value: Option<&str>) -> Result<Option<LinuxColorSpace>, String> {
    match value {
        None => Ok(None),
        Some("rec709") => Ok(Some(LinuxColorSpace::Rec709)),
        Some("srgb") => Ok(Some(LinuxColorSpace::Srgb)),
        Some(_) => Err("ScreenCapture.start colorSpace must be 'srgb' or 'rec709'".to_string()),
    }
}

fn emit_linux_start_option_diagnostics(
    options: &NormalizedStartOptions,
    lifecycle_cb: &PwLifecycleCb,
) {
...
fn emit_lifecycle(inner: &CaptureInner, kind: &str, message: &str) {
    let state = lock_state(inner);
    let Some(cb) = state.lifecycle_callback.as_ref() else {
        return;
    };
    cb(kind.to_string(), message.to_string());
}
...
fn build_linux_screen_pool(width: u32, height: u32) -> Result<Arc<LinuxFrameBufferPool>, String> {
    ...
    LinuxFrameBufferPool::new(bytes_per_buffer)
        .map_err(|err| format!("Failed to build Linux screen capture frame pool: {err}"))
}
...
mod tests {
    ...
    fn start_options_reject_invalid_capture_rect() {
        ...
        assert!(err.contains("captureRect"));
    }
}
