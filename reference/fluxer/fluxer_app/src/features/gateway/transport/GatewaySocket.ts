...
export class GatewaySocket extends EventEmitter<GatewaySocketEvents> {
	...
	private async buildGatewayUrl(): Promise<string> {
		// In the Tauri desktop shell, the gateway WebSocket is bridged through
		// the loopback proxy (the official gateway origin is presented
		// server-side). Use the proxy's gateway base when present, keeping the
		// official URL elsewhere in the UI.
		const proxyGateway =
			typeof window !== 'undefined' ? window.__FLUXER_PROXY__?.gateway : undefined;
		const url = new URL(proxyGateway ?? this.gatewayUrlBase);
