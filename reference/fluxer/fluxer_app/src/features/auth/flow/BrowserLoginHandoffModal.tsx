...
const BrowserLoginHandoffModal = observer(
	({onSuccess, targetWebAppUrl, prefillEmail}: BrowserLoginHandoffModalProps) => {
		...
		const handleOpenBrowser = useCallback(async () => {
			// Gate on a live code: without this, clicking "Open browser" while
			// code generation failed (or is still loading) opened a normal login
			// page with no way to complete the handoff — indistinguishable from
			// "the browser login button doesn't do anything useful". Surface the
			// failure state instead (HandoffCodeDisplay's error/retry UI) rather
			// than opening the browser blind.
			if (!handoffCode) {
			...
			// Copy the code to the clipboard before opening the browser so it's
			// already on the user's clipboard, ready to paste — the reported
			// friction point was landing on a normal-looking login page with no
			// obvious next step; this removes the "go back and read the code"
			// round-trip entirely.
			await TextCopyCommands.copy(i18n, handoffCode, true);
			...
			>
				...
				<Modal.Content data-flx="auth.flow.browser-login-handoff-modal.modal-content">
					<Modal.ContentLayout className={styles.content} data-flx="auth.flow.browser-login-handoff-modal.content">
						<Modal.Description data-flx="auth.flow.browser-login-handoff-modal.description">
							<Trans>
								Your code is copied automatically when you open the browser — sign in there, then paste it to link
								your account.
							</Trans>
						</Modal.Description>
						{canSwitchInstanceUrl ? (
				...
				<Modal.Footer data-flx="auth.flow.browser-login-handoff-modal.modal-footer">
					...
					<Button
						...
						disabled={!handoffCode && !isGenerating && !error}
