...
export const FluxerWordmark = observer(({variant = 'default', ...props}: FluxerWordmarkProps) => {
	...
	// Prefer the bootstrap-derived PRODUCT_NAME (stable, set once at load from
	// the instance branding — "Ruxer" in this build). RuntimeConfig.productName
	// can be re-hydrated from a live instance fetch, so it's not reliable for the
	// rebrand; PRODUCT_NAME is.
	const productName = PRODUCT_NAME || RuntimeConfig.productName;
	...
	if (productName !== DEFAULT_APP_PUBLIC_CONFIG.branding.product_name && productName !== 'Fluxer') {
