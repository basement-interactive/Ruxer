...
function resolveUrl(state: RuntimeState, path: string, query: RestRequestOptions['query']): string {
	...
	return redirectThroughDesktopProxy(url);
}

/**
 * When running inside the Tauri desktop shell, the webview cannot reach the
 * Fluxer API cross-origin (the API only allows the official web origin). The
 * native side runs a loopback reverse proxy that presents the official origin
 * server-side. We keep the official endpoint values everywhere in the UI and
 * only redirect the actual request bytes to the proxy here, preserving the full
 * path + query so the proxy can forward them onto the official origin.
 *
 * There are two kinds of redirected request:
 *
 *   1. **API calls** — issued with a relative path (`/api/v1/...`) that resolves
 *      against the webview's own origin (`window.location.origin`). The proxy
 *      forwards these onto its default upstream (the official web API origin).
 *
 *   2. **Absolute off-origin calls** — e.g. a presigned screenshare-preview
 *      upload to `https://uploads.fluxer.app/v1/relay/...`. These target a
 *      DIFFERENT host than the API. If we collapsed them to a bare path the
 *      proxy would (wrongly) send them to the API origin → 405, and the
 *      presigned `?t=` signature (bound to the uploads host) would be rejected.
 *      So we preserve the real target host by encoding it into a synthetic
 *      `/__up/<origin>/<path>` prefix the proxy decodes and forwards verbatim.
 */
function redirectThroughDesktopProxy(url: URL): string {
	const proxy = typeof window !== 'undefined' ? window.__FLUXER_PROXY__ : undefined;
	if (!proxy?.base) return url.toString();
	try {
		const base = new URL(proxy.base);
		// Same-origin requests (already loopback) pass through untouched.
		if (url.origin === base.origin) return url.toString();
		const pathSuffix = url.pathname + url.search + url.hash;
		// A relative API call resolves against the webview origin — route it to
		// the proxy's default upstream by forwarding just the path.
		if (typeof window !== 'undefined' && url.origin === window.location.origin) {
			return new URL(pathSuffix, base).toString();
		}
		// An absolute off-origin call (presigned upload, alternate host): keep
		// the real target host so the proxy forwards to the correct upstream and
		// preserves the host-bound presigned signature.
		const encodedOrigin = encodeURIComponent(url.origin);
		return new URL(`/__up/${encodedOrigin}${pathSuffix}`, base).toString();
	} catch {
		return url.toString();
	}
}
...
function isOffOrigin(url: string): boolean {
	try {
		...
		// The Tauri desktop build redirects API requests to a loopback reverse
		// proxy (a different origin than the webview). That proxy IS the trusted
		// Fluxer API path, so it must be treated as same-origin — otherwise the
		// Authorization token and X-Fluxer-Features header are dropped and every
		// authenticated REST call (e.g. message fetches) returns 401.
		const proxyBase = typeof window !== 'undefined' ? window.__FLUXER_PROXY__?.base : undefined;
		if (proxyBase) {
			try {
				if (target === new URL(proxyBase).origin) return false;
			} catch {
				// ignore malformed proxy base
			}
		}
		return true;
	} catch {
