...
// Assets are Ogg Vorbis, NOT mp3: WebKitGTK (Linux webview) has no built-in
// MP3 decoder — it delegates to GStreamer and MP3 needs the libav/ugly plugins
// that aren't guaranteed present, so `audio.play()` rejected with "The
// operation is not supported" on Linux. Ogg Vorbis is decoded by
// gst-plugins-base, which WebKitGTK effectively always ships, so this plays in
// dev and in the AppImage on every distro with no proprietary codec.
import cameraOffSound from '@app/media/sounds/camera-off.ogg';
import cameraOnSound from '@app/media/sounds/camera-on.ogg';
import deafSound from '@app/media/sounds/deaf.ogg';
import sameChannelMessageSound from '@app/media/sounds/in-channel-notification.ogg';
import incomingRingSound from '@app/media/sounds/incoming-ring.ogg';
import messageSound from '@app/media/sounds/message.ogg';
import muteSound from '@app/media/sounds/mute.ogg';
import streamSound from '@app/media/sounds/stream-start.ogg';
import streamStopSound from '@app/media/sounds/stream-stop.ogg';
import undeafSound from '@app/media/sounds/undeaf.ogg';
import unmuteSound from '@app/media/sounds/unmute.ogg';
import userJoinSound from '@app/media/sounds/user-join.ogg';
import userLeaveSound from '@app/media/sounds/user-leave.ogg';
import userMoveSound from '@app/media/sounds/user-move.ogg';
import viewerJoinSound from '@app/media/sounds/viewer-join.ogg';
import viewerLeaveSound from '@app/media/sounds/viewer-leave.ogg';
import voiceDisconnectSound from '@app/media/sounds/voice-disconnect.ogg';
...
const playOneShotSound = async (
	...
	// Native playback path (Linux): the desktop shell decodes+plays the sound
	// through the OS audio stack because WebKitGTK's <audio>/WebAudio path fails
	// with "The operation is not supported". Only used when the shell advertises
	// it AND there's no user CUSTOM sound for this type (custom sounds live as
	// blobs in the webview's IndexedDB and aren't known to the native side — those
	// keep the webview path). Fire-and-forget: returns null (no HTMLAudioElement),
	// which callers already handle for the autoplay-blocked case.
	const shell = (globalThis as {electron?: {nativeAudioAvailable?: boolean; playSound?: (name: string, volume?: number) => Promise<void>}}).electron;
	if (shell?.nativeAudioAvailable && shell.playSound && !(await CustomSoundDB.getCustomSound(type))) {
		void shell.playSound(type, clamp(volume, 0, MAX_EFFECTIVE_VOLUME)).catch((error) => {
			logger.warn(`Native sound playback failed for ${type}:`, error);
		});
		return null;
	}
