...
function probeRawCapabilities(): RawProbeResult {
	...
	try {
		// On WebKitGTK (Linux desktop) `RTCRtpSender` is UNDEFINED — the webview
		// has no WebRTC (which is exactly why the native voice engine exists).
		// The `?.` guards a missing `.getCapabilities` METHOD but NOT a missing
		// `RTCRtpSender` global, so a bare reference throws ReferenceError →
		// the old code fell into the catch and degraded to VP8/H.264. But the
		// actual video encode on the native path is done by livekit natively,
		// which supports the full modern set. When there's no in-webview WebRTC
		// to probe, report the codecs the NATIVE engine can encode (VP8/VP9/
		// H.264/AV1) rather than the webview's absent capabilities — otherwise
		// native screen share needlessly falls back to VP8.
		const senderGlobal = typeof RTCRtpSender !== 'undefined' ? RTCRtpSender : undefined;
		const capabilities = senderGlobal?.getCapabilities?.('video');
		if (!senderGlobal) {
			// Native engine (livekit + stock libwebrtc) software-encodes VP8,
			// VP9 and H.264 reliably. AV1/H.265 are left off here — advertising
			// a codec the native encoder can't actually produce would let
			// negotiation pick it and break the screen-share publish. (When the
			// native bridge's real getHardwareEncoderCapabilities lands, source
			// the exact set from it instead of this conservative default.)
			logger.debug('RTCRtpSender unavailable (no in-webview WebRTC); using native-engine codec set');
			caps.vp8 = true;
			caps.vp9 = true;
			caps.h264 = true;
