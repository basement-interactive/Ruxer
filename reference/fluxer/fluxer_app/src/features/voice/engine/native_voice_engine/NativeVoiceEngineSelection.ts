...
function isNativeVoiceEngineRequired(): boolean {
	// Tauri desktop build: we run native-desktop UX but keep the web/LiveKit
	// voice engine (no native voice_engine_v2 bridge is shipped). Returning
	// false here keeps the working web voice path and avoids a fatal crash on
	// startup from a missing native bridge.
	if (window.electron && !window.electron.voiceEngine) return false;
