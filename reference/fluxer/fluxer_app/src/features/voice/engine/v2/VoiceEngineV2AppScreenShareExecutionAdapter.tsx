...
// Thrown when the native voice engine is selected (so all voice media is routed
// through the native flow) but the native bridge does not implement the
// screen-share capability. There is no working fallback in this state: the JS
// getDisplayMedia path publishes to the webview's own LiveKit Room, which does
// not exist when voice runs in the native engine (see
// VoiceEngineV2AppConnectionHostAdapter — the LiveKit Room is only created on
// the JS voice path). Rather than throw a raw publication error that logs at
// error level and surfaces a generic "screen share failed" modal, we raise a
// NotSupportedError so handleScreenShareError routes it to the clean
// "screen sharing not supported" modal with no console-error spam.
export const NATIVE_VOICE_SCREEN_SHARE_UNSUPPORTED_MARKER = 'nativeVoiceScreenShareUnsupported' as const;

export function isNativeVoiceScreenShareUnsupportedError(error: unknown): boolean {
	return (
		error instanceof Error &&
		(error as Error & {[NATIVE_VOICE_SCREEN_SHARE_UNSUPPORTED_MARKER]?: boolean})[
			NATIVE_VOICE_SCREEN_SHARE_UNSUPPORTED_MARKER
		] === true
	);
}

function buildNativeScreenShareUnsupportedError(context: string): Error {
	const error = new Error(
		`Screen sharing is not yet supported while the native voice engine is active (${context}). ` +
			'Screen share requires the native screen-share bridge, which this desktop build does not implement.',
	) as Error & {[NATIVE_VOICE_SCREEN_SHARE_UNSUPPORTED_MARKER]?: boolean};
	// NotSupportedError so handleScreenShareError routes it through the clean
	// "screen sharing not supported" modal path instead of the generic failure
	// path; the marker lets that handler pick the native-voice-specific copy.
	error.name = 'NotSupportedError';
	error[NATIVE_VOICE_SCREEN_SHARE_UNSUPPORTED_MARKER] = true;
	return error;
}
...
class VoiceEngineV2AppScreenShareExecutionAdapter extends Store {
	...
	private async assertNativeScreenShareBridgeAvailable(context: string): Promise<void> {
		...
		// Native voice is selected but the native bridge lacks screen-share
		// capability. Falling back to the JS getDisplayMedia path is not viable
		// here: that path publishes into the webview LiveKit Room, which is never
		// created in native-voice mode (the native engine owns the voice
		// connection). Surface a clean, accurate "not supported" error instead of
		// spamming an uncaught publication error to the console.
		logger.warn('Native voice engine selected without native screen-share bridge support', {context});
		throw buildNativeScreenShareUnsupportedError(context);
	}
