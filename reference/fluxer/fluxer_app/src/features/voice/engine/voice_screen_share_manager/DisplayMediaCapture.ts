...
function getNativePreviewBridgeOptions(
	...
	// The self-preview is a small on-screen tile, NOT a second broadcast. The
	// published stream is captured+encoded entirely natively (voice_engine_publish_
	// screen); this bridge only feeds the sharer's own preview tile. Capturing it
	// at the source's full resolution/fps ran a SECOND full-res desktop capture
	// (huge CPU/GPU cost) — cap it hard to a poster-like feed, and let it idle when
	// the app is unfocused. `chromeMediaSource:'desktop'` honours these caps in the
	// capturer, so no extra canvas downscale is needed.
	...
	const scale = Math.min(1, PREVIEW_MAX_DIM / Math.max(sourceWidth, sourceHeight));
	return {
		maxWidth: Math.max(1, Math.round(sourceWidth * scale)),
		maxHeight: Math.max(1, Math.round(sourceHeight * scale)),
