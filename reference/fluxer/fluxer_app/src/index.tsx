...
// Install the Tauri-backed window.electron shim before anything reads it.
// Call the export explicitly: a bare side-effect import is tree-shaken because
// package.json marks only *.css as having side effects.
import {installDesktopTauriShim} from '@app/desktop-tauri-shim';
...
async function bootstrapApp(): Promise<void> {
	...
	const {OnboardingGate} = await loadLazyModule(
		() => import('@app/features/onboarding/OnboardingFlow'),
	);
	const {UiEditorGate} = await loadLazyModule(
		() => import('@app/features/ui_editor/UiEditorGate'),
	);
	...
	mountRoot(
		<>
			<App data-flx="index.bootstrap.app" />
			<OnboardingGate />
			<UiEditorGate />
		</>,
		'index.bootstrap',
