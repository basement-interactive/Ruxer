...
// NVENC availability probe below uses dlopen/dlsym/dlclose. Those are POSIX
// (libdl) only; MSVC has no <dlfcn.h>. On Windows the NVENC entry points live
// in nvEncodeAPI64.dll (shipped with the NVIDIA driver, same name the SDK's
// NvEncoder.cpp LoadLibrary()s). Map the three dl* calls this file makes onto
// the Win32 loader so the probe body stays identical across platforms.
#if defined(WIN32) || defined(_WIN32)
#include <windows.h>
namespace {
inline void* flx_nvenc_dlopen() {
  return reinterpret_cast<void*>(LoadLibraryW(L"nvEncodeAPI64.dll"));
}
inline void* flx_nvenc_dlsym(void* h, const char* name) {
  return reinterpret_cast<void*>(
      GetProcAddress(reinterpret_cast<HMODULE>(h), name));
}
inline void flx_nvenc_dlclose(void* h) {
  FreeLibrary(reinterpret_cast<HMODULE>(h));
}
}  // namespace
#define FLX_NVENC_DLOPEN() flx_nvenc_dlopen()
#define FLX_NVENC_DLSYM(h, name) flx_nvenc_dlsym((h), (name))
#define FLX_NVENC_DLCLOSE(h) flx_nvenc_dlclose((h))
#else
#include <dlfcn.h>
#define FLX_NVENC_DLOPEN() dlopen("libnvidia-encode.so.1", RTLD_LAZY)
#define FLX_NVENC_DLSYM(h, name) dlsym((h), (name))
#define FLX_NVENC_DLCLOSE(h) dlclose((h))
#endif
...
bool NvidiaVideoEncoderFactory::IsSupported() {
  ...
  void* hModule = FLX_NVENC_DLOPEN();
  if (!hModule) {
    RTC_LOG(LS_WARNING) << "NVENC library not found, "
                           "hardware encoding unavailable.";
    return false;
  }
  auto NvEncodeAPIGetMaxSupportedVersion =
      (NVENCSTATUS(NVENCAPI*)(uint32_t*))FLX_NVENC_DLSYM(
          hModule, "NvEncodeAPIGetMaxSupportedVersion");
  auto NvEncodeAPICreateInstance =
      (NVENCSTATUS(NVENCAPI*)(NV_ENCODE_API_FUNCTION_LIST*))FLX_NVENC_DLSYM(
