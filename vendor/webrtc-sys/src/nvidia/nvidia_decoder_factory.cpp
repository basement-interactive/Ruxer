...
// NVDEC runtime-availability probe below dlopen()s the CUVID library. libdl is
// POSIX-only; on Windows the runtime lives in nvcuvid.dll (ships with the
// driver) and is opened via LoadLibrary. Map the two dl* calls this file makes
// onto the platform loader so the probe body is identical across platforms.
#if defined(WIN32) || defined(_WIN32)
#include <windows.h>
namespace {
inline void* flx_nvdec_dlopen() {
  return reinterpret_cast<void*>(LoadLibraryW(L"nvcuvid.dll"));
}
inline void flx_nvdec_dlclose(void* h) {
  FreeLibrary(reinterpret_cast<HMODULE>(h));
}
}  // namespace
#define FLX_NVDEC_DLOPEN(name) flx_nvdec_dlopen()
#define FLX_NVDEC_DLCLOSE(h) flx_nvdec_dlclose((h))
#else
#include <dlfcn.h>
#define FLX_NVDEC_DLOPEN(name) dlopen((name), RTLD_LAZY | RTLD_LOCAL)
#define FLX_NVDEC_DLCLOSE(h) dlclose((h))
#endif
...
bool IsNvdecRuntimeAvailable() {
  void* hModule = FLX_NVDEC_DLOPEN(kNvdecRuntimeLibrary);
  if (!hModule) {
