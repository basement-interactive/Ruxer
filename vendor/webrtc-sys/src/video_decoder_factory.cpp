...
// FLUXER: FLX_NVDEC_DISABLED gates OUT the NVIDIA hardware DECODER (NVDEC). The
// decoder makes direct cuvid* calls that need nvcuvid.lib at link time, which
// ships only with the standalone NVIDIA Video Codec SDK — NOT the base CUDA
// Toolkit. On Windows we enable NVENC (encode, fully dynamically loaded, no
// import lib) but disable NVDEC so no extra SDK is required; remote-video decode
// uses the software path. Linux keeps NVDEC (its lazy-load trampolines resolve
// libnvcuvid.so at runtime — no link-time lib needed).
#if defined(USE_NVIDIA_VIDEO_CODEC) && !defined(FLX_NVDEC_DISABLED)
